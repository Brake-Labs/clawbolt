...
| `PROMPT_CACHE_IDLE_SECONDS` | unset | Idle gap after which the prompt cache counts as expired, so the next message is a cold start. Unset follows `LLM_CACHE_EXTENDED_TTL`: 3600 when it is on, 300 when off |
