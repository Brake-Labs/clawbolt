...
| `PROMPT_CACHE_IDLE_SECONDS` | unset | Idle gap after which the prompt cache counts as expired, so the next message is a cold start. Unset follows the history cache lifetime, `LLM_CACHE_HISTORY_TTL` (after its cap): 3600 for `1h`, 300 for `5m` |
| `PROMPT_STABLE_PREFIX_ENABLED` | `true` | Send memory and tool guidelines in the cached system block instead of re-sending them uncached on every message. SOUL.md, USER.md and MEMORY.md are snapshotted at each cold start; an edit made after that reaches the model as a short list of changed lines on the next message, and joins the snapshot at the next cold start |
| `COLD_START_COMPACTION_ENABLED` | `false` | On a cold start, rebuild the conversation history to `COLD_START_HISTORY_BUDGET_TOKENS`: results of read-only tool calls older than `COLD_START_VERBATIM_TURNS` turns become one-line stubs, results of calls that write stay verbatim at any age, all messages are kept, and only if what remains is over budget are the oldest turns dropped and compacted into memory. Between cold starts the history is append-only. Trimming at `CONTEXT_TRIM_TRIGGER_TOKENS` still applies as a ceiling. Replay a model comparison both ways before turning it on |
| `COLD_START_HISTORY_BUDGET_TOKENS` | `30000` | Token budget for the history a cold start rebuilds |
| `COLD_START_VERBATIM_TURNS` | `4` | Turns before the cold start whose tool results stay verbatim, if the budget allows |
...
| `MODEL_COMPARISON_MAX_CONCURRENT_RUNS` | `2` | Comparison runs in flight across all users. One run per user is enforced separately; this bounds the total, since runs share the provider rate limit with live inbound traffic |
