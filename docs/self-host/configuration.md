...
| `LLM_EVAL_CONCURRENCY` | `4` | Turns replayed in parallel by an admin model-comparison run (`AUTH_MODE=multi_user`). Each turn issues one call for the candidate, and a second for the incumbent when the run replays it, so this competes with live traffic for the provider rate limit |
