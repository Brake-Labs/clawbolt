...
| `COLD_START_COMPACTION_ENABLED` | `false` | On a cold start, rebuild the conversation history to `COLD_START_HISTORY_BUDGET_TOKENS`: tool results older than `COLD_START_VERBATIM_TURNS` turns become one-line stubs, all messages are kept, and only if the messages alone are over budget are the oldest turns dropped and compacted into memory. Between cold starts the history is append-only. Trimming at `CONTEXT_TRIM_TRIGGER_TOKENS` still applies as a ceiling. Replay a model comparison both ways before turning it on |
